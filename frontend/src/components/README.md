...
- **`DashboardView.tsx`** / **`CsvImportView.tsx`** — screen-level containers. `DashboardView` owns `filters`/modal-open state and composes `SummaryCards`+`FilterBar`+`ExpenseTable`+`ExpenseForm`. `CsvImportView` owns the parsed-result state and switches between `CsvDropzone` and `CsvReviewTable`. `App.tsx` just toggles between the two.

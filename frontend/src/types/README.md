...
- **`api.ts`** — mirrors `backend/app/schemas/{expense,category}.py` field-for-field, including `CategorySummaryItem`/`ExpenseSummaryResponse` (`GET /expenses/summary`, spec 06). Kept as a hand-written mirror rather than a generated client: the API surface is small (7 endpoints) and stable behind an approved spec (`.github/specs/01_api_contract.spec.md`), so codegen would be more machinery than the problem needs. If the backend schemas change, update this file to match — the two are meant to be read side by side.

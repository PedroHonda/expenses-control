...
- **`database.py`** — `connect_to_mongo()` / `close_mongo_connection()`, called from `app/main.py`'s FastAPI lifespan. Creates PyMongo's native async client (`pymongo.AsyncMongoClient` — not Motor; see `../../learning/001_fastapi_mongodb_setup.md`) and calls `beanie.init_beanie(...)` to register the `Expense`, `Category`, and `PaymentMethod` document models (this is what makes `Expense.find(...)`-style queries work).
- **`default_categories.json`** — the seed list of default categories, read by `scripts/seed_categories.py`. Each entry is `{"name": str, "exclude_from_total": bool}` (see `.github/specs/03_category_settings.spec.md`) — `Payment/Refund` ships with `exclude_from_total: true` so it's pre-excluded from the dashboard Total from the first seed, no admin-page step required. Deliberately **not** hardcoded in Python: per the approved spec (`.github/specs/01_api_contract.spec.md` §2.2/§6), the user can edit this file before first running the seeder to use their own default category set instead of the shipped one.
- **`default_payment_methods.json`** — same idea, for `PaymentMethod`: `{"name": str, "is_default_for_import": bool}`, read by `scripts/seed_payment_methods.py`. Exactly one entry ships with `is_default_for_import: true` (Nubank) — see `.github/specs/08_payment_methods.spec.md`.

...
async def test_upload_csv_parses_without_persisting_anything(client: AsyncClient) -> None:
    ...
    # payment_method isn't a CSV column here -- every row gets filled with
    # the configured CSV-import default (Nubank, per seeded_payment_methods).
    assert all(r["payment_method"] == "Nubank" for r in body["rows"])
...
async def test_import_batch_rejects_unknown_payment_method(client: AsyncClient) -> None:
    payload = {
        "expenses": [
            {
                "date": "2026-08-01",
    ...
    assert response.status_code == 422
    assert "NotReal" in response.json()["detail"][0]["errors"][0]

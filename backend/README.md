...
- **ReportLab for PDF export**: chosen over WeasyPrint specifically because WeasyPrint needs native Pango/Cairo/GDK-Pixbuf libraries that are painful to install on Windows (this project's primary dev environment); ReportLab is pure Python and its `platypus` layer handles multi-page tables with a repeating header row out of the box. See `.github/specs/09_pdf_monthly_report.spec.md`.
